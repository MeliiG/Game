"""Este módulo contiene la implementación del juego Serpientes y Escaleras."""
import random
...
class SerpientesYEscaleras:
    ...
    def mostrar_tablero(self):
        ...
        for cabeza_serpiente, cola_serpiente in self.serpientes.items():
            ...
            if fila_cola % 2 != 0:
                col_cola = 7 - col_cola
            tablero_copia[fila_cabeza][col_cabeza] = "\033[92m  X  \033[0m"
            tablero_copia[fila_cola][col_cola] = "\033[92m  x  \033[0m"
        for base_escalera, cima_escalera in self.escaleras.items():
            ...
            if fila_cima % 2 != 0:
                col_cima = 7 - col_cima
            tablero_copia[fila_base][col_base] = "\033[93m  W  \033[0m"
            tablero_copia[fila_cima][col_cima] = "\033[93m  w  \033[0m"
        tablero_copia[fila_jugador][col_jugador] = "\033[95m  ★  \033[0m"  # del jugador en el tablero copiado con un estrella

        print("╔═══════════════════════════════════════════════╗")
        for fila in reversed(range(8)):  # Imprimir el tablero en orden descendente
            fila_str = "║" + "║".join(str(celda).center(5) for celda in tablero_copia[fila]) + "║"
            print(fila_str)
            if fila != 0:
                print("╟─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────╢")
        print("╚═══════════════════════════════════════════════╝")
    ...
    def lanzar_dado(self):
        ...
        print("\033[94m" + caras_dado[numero - 1] + "\033[0m")
        print("\033[92m" + f"Has sacado un {numero}:" + "\033[0m")
    ...
    def cargar_historial_jugadores(self):
        ...
        try:
            with open("historial_jugadores.txt", "r", encoding="utf-8") as archivo:
    ...
    def guardar_historial_jugadores(self):
        ...
        with open("historial_jugadores.txt", "w", encoding="utf-8") as archivo:
            jugadores_ordenados = sorted(self.jugadores.items(), key=lambda x: x[1]["Puntaje"][-1] if x[1]["Puntaje"] else 0, reverse=True)
            for jugador, datos in jugadores_ordenados[:5]:  # Limitar a los primeros 5 jugadores
                total_turnos = datos["Turnos Jugados"]
                puntaje_actual = datos["Puntaje"][-1] if datos["Puntaje"] else 0  # Obtener el puntaje actual del jugador
